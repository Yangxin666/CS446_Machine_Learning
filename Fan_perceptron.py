...
#DATA_PATH = "/Users/fanyangxin/Desktop/Machine_Learning/" #TODO: if you are working somewhere other than the csug server, change this to the directory where a7a.train, a7a.dev, and a7a.test are on your machine
...
def perceptron(train_ys, train_xs, dev_ys, dev_xs, args):
    ...
    while k < args.iterations:
        for i in range(len(train_ys)):
            if np.dot(weights.transpose(), train_xs[i])<0:
                y_est = -1
            if np.dot(weights.transpose(), train_xs[i])>=0:
                y_est = 1
            #if np.dot(weights.transpose(), train_xs[i])==0:
            #    y_est = 0
            if y_est != train_ys[i]:
                weights += args.lr * train_ys[i] * train_xs[i]
        k += 1
    return weights
...
def test_accuracy(weights, test_ys, test_xs):
    ...
    for i in range(len(test_ys)):
        if np.dot(weights.transpose(), test_xs[i])<0 and test_ys[i] == 1:
            false_count += 1
        if np.dot(weights.transpose(), test_xs[i])>0 and test_ys[i] == -1:
            false_count += 1
    if np.linalg.norm(weights,2) != 0:
        accuracy = 1 - (false_count/len(test_ys))
    return accuracy
